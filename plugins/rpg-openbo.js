let handler = async (m, { conn, usedPrefix }) => {
  ...
  if (new Date() - global.db.data.users[m.sender].lastmisi > 3600000) {
    ...
    let rokit = `📲 Orderan Masuk dari [ Om Finzy ]

Kamu Dan Om Finzy Memboking Hotel
▒▒[ᴏʏᴏ]▒▒
▒▒▄▄▄▒▒ Kalian Berdua Masuk Ke kamar
▒█▀█▀█▒ kamu Membuka bh mu
░█▀█▀█░ Tete Mu diremas oleh om Finzy
░█▀█▀█░  ( . )( . )
███████.  | 🤚 |


Om Finzy Mulai Memasukan Kelamin nya ke dalam vagina mu....
`.trim();
...
Rahim mu terasa hangat ahhh💦👙
`.trim();

    let rokit3 = `Om Finzy pun crott 
...
`.trim();
    ...
    let rokit4 = `Om Finzy Memberimu Uang Lebih karena Goyanganmu Sangat unik 😝
`.trim();
