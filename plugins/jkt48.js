...
let handler = async (m, { conn, usedPrefix, text, args, command }) => {
  ...
  if (!count) {
    try {
      var tio = await fetch(`https://api.lolhuman.xyz/api/pinterest2?apikey=${api.Lol}&query=${query}`);
    ...
    try {
      let url = `https://api.lolhuman.xyz/api/pinterest2?apikey=${api.Lol}&query=${query}`;
